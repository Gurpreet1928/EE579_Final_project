...
public class server {
	...
	public static void main(String[] args) {
        connectDB();
        int port = 9776;                                            //port used (a variable)
        ...
        try {
            ...
            else if(messagefromclient.equals("MESSAGEB"))
            {
        		...
        		{
        		    String  room_no = mainResultset.getString(1);
        		    int flag= mainResultset.getInt(2);
        		    JSONObject jObj = new JSONObject();

        		    jObj.put("Room no", room_no);
        		    jObj.put("Flag", flag);
        		    jArray.add(jObj);
        		}
        		JSONObject jObjDevice = new JSONObject();
        		jObjDevice.put("Library_db", jArray);
        		
        		System.out.println(jObjDevice.values());
